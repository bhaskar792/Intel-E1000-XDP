...
static void e1000_tx_timeout(struct net_device *dev, unsigned int __always_unused txqueue);
				    ...
				    struct net_device *netdev)
{
	...
	if (count) {
		...
		// if (!skb->xmit_more ||
		// Bhaskar above line removed from kernel
		if (!netdev_xmit_more() ||
